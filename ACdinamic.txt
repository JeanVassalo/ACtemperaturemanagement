class person { //CRIANDO A CLASSE PEESOA
  constructor(name, temperature_preference, turnon_when) { //CONSTUTOR DA CLASSE, CADA ATRIBUTO DA CLASSE TEM SEU OBJETIVO
    this.name = name; //NOME DE CADA PESSOA QUE SERÁ INSTANCIADA
...
class room { //CRIANDO A CLASSE SALA
  constructor(room_number, room_temperature, people_in, ac_temperature, on_off) {
    ...
    this.room_number = 121; //NESSE ATRIBUTO FOI INSTANCIADO ESTATICAMENTE POIS SÓ EXISTIRÁ UMA SALA NO PROBLEMA
...
do { //INICIANDO O MENU
  var options = prompt("1- Para adicionar aluno; 2- Verificar quem está na sala; 3- Status do AC; 4- Temperatura do AC; 5- Entrar na sala; 6-Sair da sala; 0-Finalizar") //O USUARIO DIGITA A OPÇÃO DESEJADA
  if (options == 1) { //ESTA OPÇÃO ADICIONA UM ALUNO
  ...
  } else if (options == 2) { //OPÇÃO PARA VER QUEM ESTA NA SALA
    if (sala121.people_in[0] != null) //SE HOUVER ALGUEM NA SALA, SERA MOSTRADO
      document.write(sala121.people_in + "<br/>");
    else //SE NAO HOUVER NINGUEM NA SALA, ESSA MENSAGEM APARECERÁ
      document.write("A sala está vazia <br/>")

  } else if (options == 6) { //OPÇÃO PARA REMOVER PESSOAS DA SALA
    var cont = 0;
    var sair = prompt("Digite quem sairá da sala.") //USUARIO DIGITA QUEM SAIRA DA SALA
    while (sala121.people_in[cont] != sair) { //ENQUANTO NAO ENCONTRAR A PESSOA Q VAI SAIR, INCREMENTA O CONTADOR
      cont++;
      if (sala121.people_in[cont] == null) { // SE NÃO ENCONTRAR A PESSOA PROCURADA, APARECE UMA MENSAGEM E DA BREAK NO WHILE
        document.write('Esta pessoa não está na sala' + "<br/>");
        break;
      }
    }
    if (sala121.people_in[cont] == sair) { //SE ENCONTRAR A PESSOA, REMOVE ELA DA SALA E SEUS ATRIBUTOS TAMBEM
      sala121.people_in.splice(cont, 1); //A FUNÇÃO SPLICE RETIRA A PESSOA DO VETOR, DEVEMOS INSERIR A POSIÇÃO DELA E A UNIDADE
    ...
    if (entrar == 'Jean') {
      ...
      document.write("Esta pessoa não está cadastrada")//CASO TENTEM INSERIR ALGUEM QUE NAO ESTA NO BD
    }
